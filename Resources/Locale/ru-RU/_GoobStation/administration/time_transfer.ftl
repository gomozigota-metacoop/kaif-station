time-transfer-panel-title = Перенос времени
time-transfer-panel-player-label = Игрок
time-transfer-panel-time = Время
time-transfer-panel-add-time = Добавить время
time-transfer-panel-set-time = Установить время
time-transfer-panel-set-time-confirm = Подтвердить
time-transfer-panel-warning-no-player = Игрок не выбран
time-transfer-panel-warning-no-job = Роль не выбрана
time-transfer-panel-warning-group-no-time = Групповое время не выбрано
time-transfer-panel-warning-add-success = Время добавлено
time-transfer-panel-warning-set-success = Время перезаписано
time-transfer-panel-warning-transfer-process = Время изменяется...
time-transfer-panel-checkbox-group = Группа
time-transfer-overall-checkbox = Overall
time-transfer-panel-search-placeholder = Искать роли


cmd-timetransferpanel-desc = Открывает меню переноса времени.
admin-player-actions-window-time-transfer = Перенос времени
time-transfer-panel-no-player-database-message = Игрока нет в базе данных

# Please change it if you really-really need to do it
time-transfer-panel-author = Translated by ILLUZORR for Kaif Station
